            ...
            for j in range(config.nSimulations):
                ...
                for i in range(0, int(config.T/config.dt)):
                    ...
                    if i % int(config.sdt/config.dt) == 0:
                        nsheets.append(nfm.Z.flatten(order='F') - np.mean(nfm.Z.flatten(order='F')))
                ...
                #     plt.show()
                Xdata.append(np.mean(nsheets, 0))
                Ydata.append(ang)
...
def train_case():
    ...
    for ci in tqdm(config.ci_range):
        ...
        model = Sequential([
                    Dense(4, input_shape=(config.N**2,)),
                    # Activation('relu'),
                    Dropout(0.2),
                    # Dense(4),
                    Activation('softmax')
...
def test_case(angle = 135):
    ...
    for ci in tqdm(config.ci_range):
        ...
        for j in range(10):
            ...
            pred = model.predict(np.mean(nsheets, 0).reshape(1, config.N**2), batch_size = 1)
            pred = np.argmax(pred)
